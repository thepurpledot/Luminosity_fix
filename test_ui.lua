...
local function CreateOptions(Frame)
    ...
    function Options.Dropdown(Title, Values, Callback)
        ...
        local Selected = Utility.new("ImageButton", {
            ...
            BackgroundTransparency = 1,
            Position = UDim2.new(1, 0, 0.5, 0),
            Size = UDim2.new(0.4, 25, 0, 24),
            ZIndex = 4,
            Image = "rbxassetid://3570695787",
            ImageColor3 = DefaultOptionColor,
            ScaleType = Enum.ScaleType.Slice,
            SliceCenter = Rect.new(100, 100, 100, 100),
            SliceScale = 0.04
        })

        local SelectedValue = Utility.new("TextLabel", {
            Name = "Value",
            Parent = Selected,
            AnchorPoint = Vector2.new(0, 0.5),
            BackgroundTransparency = 1,
        ...
        local SelectedArrow = Utility.new("ImageLabel", {
            Name = "Arrow",
            Parent = Selected,
            AnchorPoint = Vector2.new(1, 0.5),
            BackgroundTransparency = 1,
        ...
        local OptionsPadding = Utility.new("UIPadding", {
            Name = "Padding",
            Parent = OptionsList,
        ...
        local OptionsLayout = Utility.new("UIListLayout", {
            Name = "Layout",
            Parent = OptionsList,
        ...
        local function UpdateDisplayedValue()
            if Properties.Value == nil then
                ...
                SelectedValue.TextTransparency = 0.5
            else
                SelectedValue.Text = tostring(Properties.Value)
                SelectedValue.TextTransparency = 0.3
            end
        end

        local function UpdateOptionVisuals(Instant)
            for _, Info in ipairs(OptionButtons) do
                local IsSelected = Properties.Value ~= nil and Info.Value == Properties.Value
                local TargetColor = IsSelected and Luminosity.ColorScheme.Primary or DefaultOptionColor
                local TargetTransparency = IsSelected and 0 or 0.3

                if Instant then
                    Info.Button.ImageColor3 = TargetColor
                    Info.Label.TextTransparency = TargetTransparency
                else
                    Utility.Tween(Info.Button, ToggleTweenInfo, {ImageColor3 = TargetColor}):Play()
                    Utility.Tween(Info.Label, ToggleTweenInfo, {TextTransparency = TargetTransparency}):Play()
                end
            end
        end

        local function SetValue(NewValue, SkipCallback, InstantVisual)
            if Properties.Value == NewValue then
                UpdateDisplayedValue()
                UpdateOptionVisuals(InstantVisual)
                return
            end
        ...
        local function GetContentHeight()
            return OptionsLayout.AbsoluteContentSize.Y + OptionsPadding.PaddingTop.Offset + OptionsPadding.PaddingBottom.Offset
        end
        ...
        local function UpdateSizing(Open, Instant)
            ...
            if Instant then
                ...
                SelectedArrow.Rotation = Open and 180 or 0
                SelectedArrow.ImageTransparency = Open and 0 or 0.3
        ...
        local function ToggleList(State, Instant)
            local NewState = State
            if NewState == nil then
                NewState = not Properties.Open
            end

            if #OptionButtons == 0 and NewState then
                return
            end

            if Properties.Open == NewState then
                UpdateSizing(Properties.Open, Instant)
                return
            end

            Properties.Open = NewState
            UpdateSizing(Properties.Open, Instant)
        end
        ...
        local function BuildOptions()
            ClearOptionButtons()
            for Index, OptionValue in ipairs(Properties.Values) do
                local OptionButton = Utility.new("ImageButton", {
                    Name = "Option",
                    Parent = OptionsList,
                    AutoButtonColor = false,
                    BackgroundTransparency = 1,
                ...
                table.insert(OptionButtons, {
                    Button = OptionButton,
                    Label = OptionButton.Label,
                    Value = OptionValue
                })
            end

            if #Properties.Values == 0 then
                SetValue(nil, true, true)
            elseif not table.find(Properties.Values, Properties.Value) then
                SetValue(Properties.Values[1], true, true)
            else
                SetValue(Properties.Value, true, true)
            end
        ...
        return setmetatable({}, {
            ...
            __newindex = function(Self, Index, Value)
                if Index == "Title" then
                    Properties.Title = Value and tostring(Value) or "Dropdown"
                ...
                elseif Index == "Values" then
                    ...
                    if typeof(Value) == "table" then
                        for ArrayIndex, ArrayValue in ipairs(Value) do
                            NewValues[ArrayIndex] = ArrayValue
                        end
                    end
